...
1a. **BreakingNews** — Strip of material headlines (last 4h, `GET /api/news/breaking`, `useBreakingNews` polls every 2 min) for watchlist + held tickers: sentiment chip, HIGH-impact badge, ticker buttons (amber when a position is held), source/age, `REV prior → current` diff when that headline triggered an intraday rescore, and a `Strikes` button that opens the detail panel with the strike recommender auto-run (`TickerDetail focus='strikes'` → `StrikeRecommender autoRun`). Renders nothing when empty or on fetch error.
2. **WatchlistChanges** — Horizontal bar showing NEW_ENTRANT (green) and REMOVED (red) badges for the selected date
3. **Recommendations + TickerDetail** — 3:2 column layout. Left: expandable cards with signal bullet lists, price/risk row, entry/exit strategy; each card header has a `Details` button (and the expanded body a `View details` button) that opens the ticker in the right-hand detail panel (trend charts, price, signals, options flow, strike recommender; a full-screen overlay below `lg`). Cards revised by intraday news show the triggering headline (parsed from `revision_reason`'s `intraday_news[...]: ` prefix) under the header with the same `Strikes` shortcut. Header has an `All | By entry` grouping toggle (persisted to `localStorage["vela.rec_group"]`; "By entry" groups BUY/STRONG_BUY under Pre-position / Reactive / Wait headings per `entry_strategy`, unknown values get their own group, HOLD/SELL collapse into a `<details>` at the bottom — `groupByEntry` in `Dashboard.tsx`) and a `Top conviction | Recently revised` sort toggle (persisted to `localStorage["vela.rec_sort"]`) that switches the API's `sort=` param. `RecommendationCard` shows a `REV · Xh ago` chip on rows revised within the last 4h, with hover tooltip surfacing `revision_reason`. `StatusBar` shows "News Xm ago" pulled from `last_refresh.intraday_news`. On desktop the panel scrolls into view when a ticker is picked from a lower section. `/?ticker=X` preselects a ticker on load (used by Discord alert deep links).
